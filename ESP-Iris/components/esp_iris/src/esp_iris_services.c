...
esp_err_t esp_iris_ota_get_status(esp_iris_ota_status_t *out_status)
{
    ...
    if (executor_ota_status(out_status)) {
        return ESP_OK;
    }
#if CONFIG_ESP_IRIS_OTA
                              ...
                              esp_iris_job_handle_t *out_job)
{
    ...
    taskEXIT_CRITICAL(&s_services_lock);
    if (slot != NULL) iris_notify_worker(&g_iris);
                       ...
                       uint64_t received_us)
{
    ...
    const bool expired = deadline_ms != 0 &&
        started_us >= received_us &&
    ...
    if (response_size > CONFIG_ESP_IRIS_RPC_BODY_BYTES ||
        response_size > ESP_IRIS_MAX_WIRE_FRAME_SIZE - IRIS_RPC_RESPONSE_HEADER_SIZE) {
        err = ESP_ERR_INVALID_SIZE;
        response_size = 0;
    }
...
{
    if (executor_queue_next(runtime)) {
        return true;
    }
    if (executor_busy()) {
        return false;
...
uint32_t iris_services_allocated_bytes(void)
{
    ...
    return (valid_state(state) ? state->allocated_bytes : 0) +
        iris_files_allocated_bytes() + executor_allocated_bytes();
}
