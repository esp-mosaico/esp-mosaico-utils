...
def test_cli_reports_product_version(capsys) -> None:
    with pytest.raises(SystemExit) as caught:
        build_parser().parse_args(["--version"])
    assert caught.value.code == 0
    assert capsys.readouterr().out.strip() == f"esp_iris.py {__version__}"
...
def test_endpoint_maintenance_lease_ignores_stale_identity_for_unmanaged_usb(
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            assert (await aborted.json())["lease"]["state"] == "aborted"
            assert endpoint not in hub._endpoint_tasks
            assert endpoint not in hub._locks
        finally:
            await client.close()
            await hub.close()
            store.close()
...
def test_local_auth_cli_is_opt_in() -> None:
    ...
    assert default_web.discover_mdns is None  # Auto: enabled when no endpoints are explicit.
