...
UI 初始化等待上限为五秒；USB 打开 Download Ideas 的请求（含串行排队）
最多等待两秒。超时保留 USB 维护通道，过期的待处理请求不会随后打开页面，
迟到的响应也不会被后续请求误用。Wi-Fi 初始化失败会释放本次创建的驱动、
事件处理器和网络接口，保留共享事件循环、状态锁及已保存凭据；重新扫描或
通过 USB 配网会再次尝试初始化，无需为此重启设备。
