...
{
    const char *accepted[] = {
        "0.1", "0.1.0", "0.1.0-recovery", "0.0.9"
    };
    for (size_t i = 0; i < sizeof(accepted) / sizeof(accepted[0]); ++i) {
        assert(factory_recovery_version_satisfies("0.1", accepted[i]));
    }
    const char *rejected[] = {
        "0.1.1", "0.2.0", "1.0.0", "2.5.0-recovery", "2.7.0-recovery",
        "2.8.5-recovery", "2.8.6-recovery", "3.0.0",
        "", "0", "0.", "0.1.", "0.1.x", "0.1.0.1", "-1.0.0",
        "4294967296.0.0", "0.4294967296.0", "0.1.4294967296"
    };
    for (size_t i = 0; i < sizeof(rejected) / sizeof(rejected[0]); ++i) {
        assert(!factory_recovery_version_satisfies("0.1", rejected[i]));
    }
    assert(!factory_recovery_version_satisfies("0.1", NULL));
    assert(!factory_recovery_version_satisfies(NULL, "0.1"));
    assert(!factory_recovery_version_satisfies("bad", "0.1"));
    ...
    assert(factory_recovery_version_can_replace("0.1", "0.1"));
    assert(factory_recovery_version_can_replace("0.1", "0.1.1"));
    assert(!factory_recovery_version_can_replace("0.1", "0.0.9"));
    assert(!factory_recovery_version_can_replace("0.1", "2.8.5-recovery"));
    ...
    assert(factory_recovery_version_satisfies("0.1.1", "0.1"));
    return 0;
}
''',
        encoding="utf-8",
