...
def provisioning_candidate(
    ...
    *,
    ...
    read_mac = mac_reader or (
        lambda port: read_rom_hardware_mac(context, model, port, idf_path)
    )
    ...
    if len(registered_ports) > 1:
        if idf_path is not None:
            discovered = {port: read_mac(port) for port in registered_ports}
            available = ", ".join(
                f"{value} at {port}"
